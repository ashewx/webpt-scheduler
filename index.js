...
function WebhookProcessing(req, res) {
	...
	switch(intent){
		...
		case "set-pt":
			...
			text = 'SELECT d.doctorid FROM doctors AS d WHERE d.fname = ' + agent.parameters['first-name'] + ' AND d.lname = ' + agent.parameters['last-name'];
			let pt_id = null;
			client.query(text).then(response => {
				console.log(response.rows[0]);
			...
			text = 'INSERT INTO goesto VALUES(' + patient_id + ', ' + pt_id + ')';
			client.query(text).then(response => {
				console.log(response.rows[0]);
